# -*- coding: utf-8 -*-
"""该代码是对轴线方程为一般二次曲线的多毛细管透镜的模拟，前焦距为15mm，长度为66mm，后焦距为64mm，一般只对其层数进行修改模拟"""
__author__ = "xishuaibin"
__date__ = "08 Sep 2025"
...
class BentCapillary(roe.OE):
    def __init__(self, *args, **kwargs):
        ...
        s0 = self.f - self.rSample
        self.Rmax = 2.0
        self.s0 = s0
        self.isParametric = True
"""如下是多毛细管透镜的相关函数方程，可以实现不同层数毛细管透镜的构成；self.x随着不同层数改变，是入口端不同层数对应的单毛细管的中心x坐标"""
...
"""以下的E0是ID33线站XRS实验所用的X射线能量，一般无需修改；rSample是多毛细管透镜样品端的工作距离；f是样品到多毛细管出口端的距离；r0是构成多毛细管透镜的单毛细管半径；
wall是单毛细管的壁厚；layzers是多毛细管透镜的层数，可以修改；nRefl、nReflDisp和xzPrimeMax无需修改，与后续的画图有关。"""
E0 = 9689.
rSample = 15
f = 81# f-rSample=L
r0 = 0.005
wall = 0.00125
layers = 150 # number of hexagonal layers
nRefl = 24
nReflDisp = 24
xzPrimeMax = 7.

"""dx和dz是样品的水平和垂直尺寸，可以修改；其他无需修改。DiamondFSM1是放在多毛细管入口处的探测器，无需修改；DiamondFSM2是为了探测多毛细管出射端之后的光斑情况，位置可按需修改。
def build_beamline(）中的其他位置均无需修改。"""    
def build_beamline(nrays=1000):
    beamLine = raycing.BeamLine(height=0)
    rs.GeometricSource(
        beamLine, 'GeometricSource', (0, 0, 0), nrays=nrays,
        dx=0.005, dz=0.005, distxprime='annulus', dxprime=0.028, dzprime=0.028,
        ...
        for i in i6:
            for m in ms:
                ...
                capillary = BentCapillary(
                    ...
                    material=mGlass, limPhysY=[rSample, f],#y的范围决定s的范围
                    f=f, rSample=rSample, x=x,entranceAlpha=alpha, rIn=r0, rOut=1.25 * r0)
...
#    beamLine.sources[0].dxprime = 0, np.arcsin(r0 / rSample)
    #beamLine.capillaries.center = 0, 44.2, 0
    #beamLine.capillaries.pitch = 0.000087
    #beamLine.capillaries.yaw = 1
...
"""绘制多毛细管透镜的剖面图，ax1.set_ylim(- 15.0, 15.0)可根据不同层数的多毛细管透镜进行适时修改。"""
def plot2D():
    beamLine = build_beamline()
    fig1 = plt.figure(1, figsize=(8, 6))
#    ax1 = plt.subplot(111, aspect=equal', label='1')
    ax1 = plt.subplot(111, aspect=5, label='1')
    ...
    for i in beamLine.firstInLayer:
        ...
        x1 = capillary.local_x0(s)
        x2 = -capillary.local_x0(s)
        r = capillary.local_r0(s)
#        ax1.plot([0, f-s[-1]], [0, x1[-1]], 'k-', lw=0.5)
        line = ax1.plot(f-s, x1, 'k-.', lw=0.5)
        line = ax1.plot(f-s, x2, 'k-.', lw=0.5)
        line[0].set_dashes(seq)
        ax1.plot(f-s, x1-r, 'r-', lw=2)
        ax1.plot(f-s, x1+r, 'r-', lw=2)
        ax1.plot(f-s, x2-r, 'r-', lw=2)
        ax1.plot(f-s, x2+r, 'r-', lw=2)
    ax1.set_xlim(0, f)
...
#    PlotClass = xrtp.XYCPlotWithNumerOfReflections
    ...
    plot = xrtp.XYCPlot('beamSource', (1,),
                        ...
                        yaxis=xrtp.XYCAxis(r'$z$', 'mm'),persistentName='guangyuan.mat')
    plot.xaxis.fwhmFormatStr = '%.3f'
    plot.yaxis.fwhmFormatStr = '%.3f'
    plot.baseName = 'sourceSize'
    plots.append(plot)  # 光源大小

    plot = xrtp.XYCPlot(
        beam=r"beamSource",
        aspect='auto',
        xaxis=xrtp.XYCAxis(r"x", u"mm",  bins=256, ppb=2),
        yaxis=xrtp.XYCAxis(r"x'", u"mrad",bins=256, ppb=2))
    plot.xaxis.fwhmFormatStr = '%.3f'
    plot.baseName = 'sourceprime-x'
    plots.append(plot)  # 光源发散度

    plot = xrtp.XYCPlot(
        beam=r"beamSource",
        aspect='auto',
        xaxis=xrtp.XYCAxis(r"z", u"mm", bins=256, ppb=2),
        yaxis=xrtp.XYCAxis(r"z'", u"mrad",bins=256, ppb=2))
    plot.xaxis.fwhmFormatStr = '%.3f'
    plot.baseName = 'sourceprime-z'
    plots.append(plot)  # 光源发散度

 #  plot = xrtp.XYCPlot('beamFSM2', (1,3),
 #                      xaxis=xrtp.XYCAxis(r'$x$', 'mm', limits=[-1, 1]),
 #                      yaxis=xrtp.XYCAxis(r'$z$', 'mm', limits=[-1, 1]),persistentName='focal.mat')
 #  plot.xaxis.fwhmFormatStr = '%.3f'
 #  plot.yaxis.fwhmFormatStr = '%.3f'
 #  plot.baseName = 'focalSize'
 #  plots.append(plot) # 后焦点处光斑

 #  plot = xrtp.XYCPlot(
 #      beam=r"beamFSM2",
 #      aspect='auto',
 #      xaxis=xrtp.XYCAxis(r"x", u"mm", limits=[-1, 1], bins=256, ppb=2),
 #      yaxis=xrtp.XYCAxis(r"x'", u"mrad",limits=[-xzPrimeMax * 4, xzPrimeMax * 4], bins=256, ppb=2))
 #  plot.xaxis.fwhmFormatStr = '%.3f'
 #  plot.baseName = 'focalprime-x'
 #   plots.append(plot) # 后焦点处发散度

 #   plot = xrtp.XYCPlot(
 #      beam=r"beamFSM2",
 #       aspect='auto',
 #       xaxis=xrtp.XYCAxis(r"z", u"mm", limits=[-1, 1], bins=256, ppb=2),
 #       yaxis=xrtp.XYCAxis(r"z'", u"mrad",limits=[-xzPrimeMax * 4, xzPrimeMax * 4],
 #                          bins=256, ppb=2))
 #   plot.xaxis.fwhmFormatStr = '%.3f'
 #   plot.baseName = 'focalprime-z'
 #   plots.append(plot) # 后焦点处发散度

    for ibins, bins in enumerate([256]):
        plot = xrtp.XYCPlot(
            'beamFSM1', (1, 3, -1),
    ...
    #    print(capillariesToShow)
        """光线在多毛细管透镜最内层、中间层和最外层分别按反射次数和反射角度的传输分布"""
        ...
        plot = PlotClass(
            ...
            xaxis=xrtp.XYCAxis(r'$x$', 'mm', bins=bins, ppb=2),
            yaxis=xrtp.XYCAxis(r'$z$', 'mm', bins=bins, ppb=2),
            caxis='category', title='FSM2_xzCat', persistentName='shuju.mat')
        plot.xaxis.fwhmFormatStr = fwhmFormatStr3
        plot.yaxis.fwhmFormatStr = fwhmFormatStr3
        plot.xaxis.limits = [-5, 5]#[-beamLine.xzMax, beamLine.xzMax]
        plot.yaxis.limits = [-5, 5]#[-beamLine.xzMax, beamLine.xzMax]
    #    plot.fluxFormatStr = '%.2e'
        plot.baseName = 'NCapillaries-d-FSM2-xzCat{0}'.format(ibins)
        plots.append(plot)

        """多毛细管透镜出射端后位置光斑尺寸随着光线在多毛细管内反射次数的分布"""
        plot = PlotClass(
            'beamFSM2', (1, 3),
            xaxis=xrtp.XYCAxis(r'$x$', 'mm', bins=bins, ppb=2),
            yaxis=xrtp.XYCAxis(r'$z$', 'mm', bins=bins, ppb=2),
    ...
    #    plot.fluxFormatStr = '%.2e'
        ...
        """多毛细管透镜出射端后位置光斑尺寸随发散度的分布，x-x'和z-z'。"""
        plot = PlotClass(
        ...
        plot.yaxis.fwhmFormatStr = fwhmFormatStr3
        plot.xaxis.limits = [-5, 5]#[-beamLine.xzMax, beamLine.xzMax]
        plot.yaxis.limits = [-xzPrimeMax * 4, xzPrimeMax * 4]
        plot.caxis.fwhmFormatStr = None
        plot.caxis.limits = [-0.5, nReflDisp+0.5]
    #    plot.fluxFormatStr = '%.2e'
        ...
        plot = PlotClass(
            'beamFSM2', (1, 3), aspect='auto',
            xaxis=xrtp.XYCAxis(r'$z$', 'mm', bins=bins, ppb=2),
            yaxis=xrtp.XYCAxis(r"$z'$", 'mrad', bins=bins, ppb=2),
            caxis=xrtp.XYCAxis('number of reflections', '', bins=nReflDisp + 1,
                               ppb=16, data=raycing.get_reflection_number),
            title='FSM2_zzPrimeN')
        plot.xaxis.fwhmFormatStr = fwhmFormatStr3
        ...
        plot.caxis.limits = [-0.5, nReflDisp + 0.5]
        #    plot.fluxFormatStr = '%.2e'
        plot.baseName = 'NCapillaries-f-FSM2-zPhaseSpaceN{0}'.format(ibins)
        plots.append(plot)
        ...
        plot.caxis.fwhmFormatStr = None
        plot.caxis.limits = [-xzPrimeMax * 4, xzPrimeMax * 4]
    #    plot.fluxFormatStr = '%.2e'
...
"""repeats是追迹重复次数，多次累积，便于观察现象"""
def main():
    beamLine = build_beamline()
    plot2D()
    plots = define_plots(beamLine)
    xrtr.run_ray_tracing(plots, repeats=10*16, beamLine=beamLine,
                         processes=1)
